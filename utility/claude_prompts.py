...
2. By looking at the <other_aspects> block, generate <generation_instructions> block, starting it with a phrase 'I must vary the following aspects in the code snippets I'm going to generate:', and then repeating & refining all the aspects from the <other_aspects> block in order to make sure you understood the requirements.
3. Closely following all the information and adhering to all the instructions from the <refined_rule> and <generation_instructions> blocks, generate your samples as described in the <instructions>. 
4. Double-check your generated samples by counting whitespaces to make the identation correct (see rule #3).
...
3. Generate your snippets (only code, without comments) should be in the following format:
    <result>
    {{code_snippet_1}}
    --
    {{code_snippet_2}}
